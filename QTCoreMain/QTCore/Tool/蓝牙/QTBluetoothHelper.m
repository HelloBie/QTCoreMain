...
static QTBluetoothHelper *_helper;

@interface QTBluetoothHelper ()<CBCentralManagerDelegate,CBPeripheralDelegate>
@property(nonatomic,strong)CBCentralManager *manager;
@property(nonatomic,strong)NSMutableDictionary *deviceDic;
@property(nonatomic,strong)CBPeripheral *peripheral;
@property(nonatomic,strong)CBCharacteristic *characteristic;
@property(nonatomic,copy)void (^findDevices)(NSMutableDictionary * dic);
@end

@implementation QTBluetoothHelper
+ (QTBluetoothHelper *)shareHelper
{
    if (!_helper) {
        static dispatch_once_t shareBTHelperToken;
        dispatch_once(&shareBTHelperToken, ^{
            _helper = [[super allocWithZone:nil] init];
...
{
    if (!_helper.manager) {
        static dispatch_once_t BTHelperManagerToken;
        dispatch_once(&BTHelperManagerToken, ^{
                _helper.manager = [[CBCentralManager alloc] initWithDelegate:_helper queue:dispatch_queue_create("QTBluetoothHelperqueue", DISPATCH_QUEUE_CONCURRENT)];
...
{
    return [QTBluetoothHelper shareHelper];
}
